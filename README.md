...
> 📦 **版本说明（重要）**：本仓库已迁移到 **C++ 版**（`v2.0.0` 起，体积约 1MB，无需 Node 运行时），**本 Node 版（`v1.2.0`）为最后一个版本，不再维护**。Node 版源码以本 README 同目录的 `old_LanBridge.zip` 归档保留，旧 release 仍可下载。新版本请以 C++ 版为准。
...
- **📤 文件上传** — 多选 / 整文件夹 / **拖拽上传**，单文件 ≤ 10GB，流式上传 + **单文件/总体进度**
...
- **💬 实时聊天** — SSE 推送，Apple 风格气泡，图片消息，**实时在线人数**
...
- **删除**：仅**上传者本人**（同一浏览器标签）或 **localhost** 可见删除按钮；删除后自动清理空目录并同步给其他浏览器

### 文件上传

页面上传区支持多选文件、**整个文件夹**或**直接拖拽文件**，逐文件流式上传（**单文件 ≤ 10GB**）。上传时显示**每个文件的进度条 + 总体进度**，全部完成后进度面板自动隐藏；失败的文件会立即提示文件名并保留在列表中。上传成功后通过 SSE 即时同步到其他浏览器（无 SSE 时 5 秒轮询兜底）。上传路径经严格校验（拒绝 `..` 穿越与服务自身文件），失败自动清理半成品。
...
- **新增**：文件删除（仅上传者本人或 localhost）
- **新增**：上传进度（单文件 + 总体，完成后自动隐藏）
- **新增**：上传区拖拽上传
- **改进**：上传后通过 SSE 实时同步到其他浏览器（5 秒轮询兜底）
- **改进**：在线人数实时化（5 秒心跳 + 关闭页面立即移除）
- **改进**：localhost 访问免填昵称，固定显示为 `localhost`
- **修复**：中文昵称导致 HTTP 请求头非 ISO-8859-1 报错（上传 / 列表加载失败）
- **修复**：深色模式下 SHA256 / 预览按钮颜色
- **说明**：这是最后一个 Node 版本，后续以 C++ 版为准
