...
- **📋 共享剪贴板** — 应用层文本共享（双 slot），SSE 实时同步
- **🌓 深色模式** — 自动跟随系统 / 手动切换 / localStorage 记忆
- **🛡 本机管理** — localhost 限定的功能权限开关，7 项可独立启停
...
5. 保存文件，浏览器按 **Ctrl+F5** 强制刷新（dev 模式立即生效）

> **EXE 模式下**，`index.html` 已被嵌入到 EXE 里。改完要重新跑 `build.bat` 才能生效。
...
`index.html` 内的"正文区"按段落（`<section class="text-block">`）划分，每段右上角"复制本段 / 全部复制"按钮。纯前端 Clipboard API 实现，无后端。
...
独立的"共享剪贴板"区域，**应用层共享文本**（不操纵操作系统剪贴板）：

- 两个 slot（宿主机 / 虚拟机）独立存储
- 一键写入 + 一键复制到本机浏览器剪贴板 + 一键清空
- **SSE 实时同步**：任一端写入，另一端通过 SSE 立即看到
- **仅内存保存**，服务重启自动清空，不落盘
- 单条最大 20000 字符
...
- 7 个功能权限开关，**立即生效** + **SSE 广播**给所有客户端：
  - 文件下载 / 文件上传 / 在线预览 / SHA-256 / 共享剪贴板 / 局域网聊天 / 网速测试
...
```text
.
├── server.js           # Node 服务端（约 770 行，单文件，零依赖）
